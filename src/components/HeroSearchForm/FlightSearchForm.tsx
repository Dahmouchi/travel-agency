...
import { GuestsObject } from "@/types/type";
...
export const FlightSearchForm: FC<Props> = ({
  className,
  formStyle = "default",
}) => {
            ...
            <div className="relative grid gap-7 rounded-2xl bg-white p-6 shadow-lg ring-1 ring-black/5 dark:bg-neutral-800 dark:ring-white/10">
              {flightClass.map((item) => (
                <Headless.CloseButton
                  ...
                  className="-m-3 flex cursor-pointer items-center rounded-xl p-2 text-sm font-medium hover:bg-gray-50 focus:outline-hidden focus-visible:ring-3 focus-visible:ring-orange-500/50 dark:hover:bg-gray-700"
                >
                  {item.name}
                </Headless.CloseButton>
              ))}
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 rounded-full">
        ...
        <DateRangeField
          ...
          description={
            tripType === "oneWay"
              ? T["HeroSearchForm"]["Flying date"]
              : T["HeroSearchForm"]["Flying dates"]
          }
