...
import { ChevronDown, EyeOff, FileText, HelpCircle, Key, Lock, Scale, Shield, ShieldCheck } from "lucide-react";
...
export default async function CategoryToursPage() {
  ...
  <div className="absolute bottom-20 right-1/4 text-white/20 animate-bounce" style={{ animationDelay: "-3s" }}>
    <Lock className="w-10 h-10" />
  </div>
  <div className="absolute top-1/3 right-10 text-white/20 animate-bounce" style={{ animationDelay: "-1.5s" }}>
  ...
  <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
    ...
    <p className="text-lg md:text-xl text-white/90 mb-12 max-w-3xl mx-auto leading-relaxed animate-slide-up">
      Comment Happy Trip protège et utilise vos informations personnelles dans le respect de votre vie privée
    </p>
  ...
  <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white animate-bounce">
    <ChevronDown className="w-6 h-6" />
      ...
      <div className=" w-full flex items-center justify-center pt-8" id="conditions">
        <div className="lg:max-w-2/3 px-4"><SafeHTML className="text-base/8"  html={sections?.plitique || ""}/></div>
      </div>
    </div>
  );
}
