...
const TourDetailsDiscover = ({ tour, programss }: Props) => {
  ...
  const [selectedPersons, setSelectedPersons] = useState<number>(0);
    ...
    <div className="relative">
        ...
        <div className="flex flex-col-reverse lg:flex-row lg:gap-8 gap-4 items-center">
          ...
          <div className="lg:w-1/2 w-full h-full relative">
            ...
            <img
              ...
              // <Image src="/path/to/your/image.jpg" alt="..." width={600} height={400} className="rounded-xl shadow-md" />
            />
            <div className="text flex items-center gap-2 text-gray-500 absolute top-3 right-3 bg-white bg-opacity-80 px-3 py-1 rounded-full">
              <MapPinIcon className="w-4" />
              <p className="text-md ">{tour.ville}</p>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white">
        ...
        <div className="bg-[#8ebd21] text-white grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8 mb-4">
          ...
          <div className="flex items-center lg:border-r-2 border-r-0 border-b-2 lg:border-b-0 border-white lg:px-0 px-8 justify-start lg:text-nowrap text-xl md:text-lg lg:justify-center gap-2 w-full py-4  lg:my-4 ">
            <MapPinIcon className="w-7 h-7 md:w-6 md:h-6 text-white" />{" "}
            {/* Replace with moon/duration icon */}
            <span className="md:text-lg text-xl">
              Ville de départ: {tour.villeDepart}
            </span>
          </div>
          <div className="flex items-center lg:px-0 px-8 justify-start lg:text-nowrap text-xl md:text-lg lg:justify-center gap-2 w-full py-4  lg:my-4 ">
            <img
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <path
                  ...
                  strokeWidth={2}
                  d="M15 9V3H9v6H4v12h16V9h-5z"
                />
              </svg>
              <span className="whitespace-nowrap text-2xl font-bold">
                {selectedPersons > 0
                  ? `${tour.type === "INTERNATIONAL" ? "À partir de " : ""}${(
                      tour.priceDiscounted * selectedPersons
                    ).toLocaleString()} MAD`
                  : "Sélectionnez le nombre de personnes"}
              </span>
            </div>

            {/* 👥 Person Dropdown and Booking Button */}
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
        ...
        `}
              ...
              <div>
                ...
                >
                  {selectedPersons > 0 ? "Réserver" : "Sélectionnez d'abord"}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-2 lg:hidden px-2">
          <div className="bg-[#FFEDF3] rounded-xl shadow border border-slate-200 p-4 space-y-4">
            ...
            <div className="flex items-center gap-3 border-b pb-3 justify-between">
              ...
              <span className="text-gray-800">{tour.villeDepart}</span>
            </div>
...
export default TourDetailsDiscover;
...
import ReservationsFormDiscover from "./reservationsFormDiscover";
