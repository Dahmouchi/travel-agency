...
export function OptimizedSearchInput({
  ...
  const handleSelect = useCallback((tour: Tour) => {
    if (onSelect) {
      onSelect(tour)
    } else {
  ...
  const defaultResultRenderer = useCallback((tour: Tour, index: number) => (
    <li
      key={tour.id}
    ...
    <div className={`relative w-full max-w-xl z-50 ${className}`}>
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-full focus:outline-none focus:ring-1 focus:ring-blue-500 h-full pr-20"          autoComplete="off"
        />
