...
import { getTravelRequest, UpdateStatuSurMesure } from "@/actions/reservationsActions";
          ...
          <table className="min-w-full text-sm text-gray-700">
            <thead className="bg-gradient-to-r from-green-600 to-lime-500 text-white">
              <tr>
                ...
                <th className="py-3 px-4 text-left font-medium">Budget (MAD)</th>
            ...
            <tbody>
              {requests.map((req) => (
                ...
                >
                  <td className="py-3 px-4">{req.firstName} {req.lastName}</td>
                  <td className="py-3 px-4">{req.email}</td>
                  <td className="py-3 px-4">{req.destination}</td>
                  <td className="py-3 px-4">{new Date(req.departureDate).toLocaleDateString("fr-FR")} → {new Date(req.returnDate).toLocaleDateString("fr-FR")}</td>
          ...
          <DialogContent className="">
            <DialogHeader>
              ...
              <DialogDescription className="text-slate-400">
                Change the status for {selected?.name}&apos;s
                reservation
              </DialogDescription>
            </DialogHeader>
       ...
       <DialogContent className="sm:max-w-5xl">
          ...
          {selected && (
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
              <div>
                <p><span className="font-semibold">Nom :</span> {selected.title} {selected.firstName} {selected.lastName}</p>
                <p><span className="font-semibold">Email :</span> {selected.email}</p>
                <p><span className="font-semibold">Téléphone :</span> {selected.countryCode} {selected.phone}</p>
                <p><span className="font-semibold">Ville de départ :</span> {selected.departureCity}</p>
                <p><span className="font-semibold">Destination :</span> {selected.destination}</p>
                <p><span className="font-semibold">Dates :</span> {formatDate(selected.departureDate)} → {formatDate(selected.returnDate)}</p>
              </div>
              <div>
                <p><span className="font-semibold">Durée :</span> {selected.duration} jours</p>
                <p><span className="font-semibold">Budget :</span> {selected.budget.toLocaleString()} €</p>
                <p><span className="font-semibold">Catégorie :</span> {selected.accommodationCategory}</p>
                <p><span className="font-semibold">Chambres :</span> {selected.numberOfRooms}</p>
                <p><span className="font-semibold">Transport :</span> {selected.needsTransport}</p>
                <p><span className="font-semibold">Vol :</span> {selected.needsFlight}</p>
              </div>
              <div className="col-span-2 mt-3">
                <p><span className="font-semibold">Souhaits d&apos;hébergement :</span> {selected.accommodationWishes || "Aucun"}</p>
                <p><span className="font-semibold">Message :</span> {selected.message || "Aucun message"}</p>
              </div>
            </div>
          )}
