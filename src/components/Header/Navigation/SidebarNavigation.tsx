...
const SidebarNavigation: React.FC<Props> = ({
          ...
          children: [
            {
              name: "National",
              href: "#",
              children: nationalDestinations.map((d) => ({
                name: d.name,
            ...
            },
            {
              name: "International",
              href: "#",
              children: internationalDestinations.map((d) => ({
                name: d.name,
  ...
  const menuData = transformMenu(data);
  ...
  return (
    <div>
      ...
      FOR OUR DEMO <div className="mt-5">{renderSearchForm()}</div>*/}
      <ul className="flex flex-col gap-y-1 px-2 py-6">
        {menuData.map((item, index) => _renderItem(item, index))}
      </ul>
