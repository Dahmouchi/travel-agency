...
const HeroSub: FC<HeroSubProps> = ({ title, description, breadcrumbLinks }) => {
  return (
    <>
      <section
        className=" text-center bg-center  relative bg-gradient-to-b from-white from-10% dark:from-darkmode bg-cover  to-[#8EBD22] to-90% dark:to-darklight overflow-x-hidden"
        style={{ backgroundImage: "url(/images/subHero.jpg)" }}
      >
        <div className="w-full h-full text-white bg-black/20 bg-opacity-50  py-8 shadow-lg backdrop-blur-xs lg:max-sm:px-8 max-sm:px-4">
          <h2 className="text-midnight_text mt-24 lg:text-[40px] text-[30px] leading-[1.2] relative font-bold text-white dark:text-white capitalize">
            {title}
          </h2>
